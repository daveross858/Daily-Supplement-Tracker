...
export default function Home() {
  // Step 1: ALL hooks declared unconditionally
  ...
  // Step 2: ALL useEffect hooks with stable dependencies
  useEffect(() => {
    let isMounted = true
    
    async function loadData() {
      if (!isAuthenticated || !user || isDataLoaded) return
  ...
  useEffect(() => {
    ...
    const checkNewDay = () => {
      ...
      if (currentDate !== today) {
        console.log('New day detected, resetting supplements')
        setCurrentDate(today)
        setSupplements(prev => {
          const reset = prev.map(s => ({ ...s, completed: false }))
          updateUserTodaysData(user.id, reset).catch(console.error)
          return reset
        })
      }
    }
  ...
  const handleAddFromLibrary = async (libraryItem: SupplementLibraryItem, timeCategory: TimeCategory) => {
    if (!user) return
  ...
  // Step 5: EARLY RETURNS - After all hooks
  if (isLoading) {
    return (
